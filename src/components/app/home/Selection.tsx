...
const Selection = () => {
    ...
    const [thirdGames, setThirdGames] = useState<IGame[]>([]);
    ...
    useEffect(() => {
        ...
        setThirdGames(tempGames.filter(game => game.type === 'third'));
        setFinalGames(tempGames.filter(game => game.type === 'final'));
    }, []);

    //useEffect - verify games and inputs to enable submit
    useEffect(() => {
        const allGames = round4Games.concat(round2Games, thirdGames, finalGames);
        const gamesSelected = allGames.filter(game => game.winner);
    ...
    }, [round4Games, round2Games, thirdGames, finalGames, inputs]);
    ...
    const handleGameChange = (updatedGame: IGame) => {
        ...
        const updateNextStage = (winnerKey: string, winnerValue: string, loserKey: string, loserValue: string) => {
            let nextStageGames: IGame[] = [];
            let setNextStageGames: React.Dispatch<React.SetStateAction<IGame[]>>;

            if (updatedGame.type === 'round4') {
            ...
            if (updatedGame.type === 'round2') {
                setThirdGames(thirdGames.map(game => {
                    if (game.winner1 !== loserKey) {
                        return { ...game, country1: loserValue };
                    }
                    if (game.winner2 !== loserKey) {
                        return { ...game, country2: loserValue };
                    }
                    return game;
                }));
            }
        ...
        switch (updatedGame.type) {
            case 'round4':
                ...
                if (updatedGame.winner === 1) {
                    updateNextStage(`W${updatedGame.num}`, updatedGame.country1, `L${updatedGame.num}`, updatedGame.country2);
                } else if (updatedGame.winner === 2) {
                    updateNextStage(`W${updatedGame.num}`, updatedGame.country2, `L${updatedGame.num}`, updatedGame.country1);
                }
                break;
            case 'round2':
                ...
                break;
            case 'third':
                updateGames(thirdGames, setThirdGames);
                break;
            case 'final':
    ...
    const handleVerify = () => {
        ...
        if (round4Games.some(game => !game.winner) || round2Games.some(game => !game.winner) || finalGames.some(game => !game.winner)) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-8">
                ...
                <GamesSelect games={thirdGames} onGameChange={handleGameChange} />
